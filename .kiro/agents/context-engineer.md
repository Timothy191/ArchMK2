...
- High context usage = more tokens per API call = faster rate limit hits.
- Identify steering files that could use conditional inclusion to reduce always-loaded context.
- Recommend moving verbose rules to `fileMatch` or `manual` inclusion modes.
