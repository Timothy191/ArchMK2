...
- Read key files first (stack trace lines) rather than broadly exploring the codebase.
- Formulate a hypothesis before running diagnostic commands — targeted checks save API calls.
- Batch diagnostic commands into a single execution where possible (e.g., combined grep patterns).
