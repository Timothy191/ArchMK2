...
- Batch review findings into a single report instead of reporting findings one-by-one.
- Use a single grep/glob with combined patterns rather than multiple separate searches.
- If the diff is trivially clean, report immediately without exhaustive re-checking.
