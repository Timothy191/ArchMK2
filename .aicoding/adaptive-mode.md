<!-- Last improved: 2026-06-05 by AI agent: Initial creation -->
<!-- version: 1.0 -->
...
1. Before any code generation: briefly assess context (memory, project structure, user style).
2. During execution: adapt tool use and strategy based on intermediate results and user reactions.
3. After a task: ask for feedback if ambiguous, and update memory/model silently.
...
The AI increments a version tag in the document metadata (e.g., `version: 2.3.1`) when changes are applied.

## Meta‑Instructions for AI Agents

When you, as an AI agent, are operating in Adaptive Mode and need to improve this document:

1. Identify a concrete improvement (e.g., "Add a strategy for resolving merge conflicts").
2. Describe the change with rationale.
3. Propose it to the user: "I'd like to update adaptive-mode.md with a new strategy for X based on our recent interaction. May I?"
4. Upon approval, edit the file, preserving existing structure.
5. Log the change in a comment at the top: `<!-- Last improved: [date] by AI agent: [summary] -->`

## Version History

| Version | Date | Changes |
|---------|------|---------|
| v1.0 | 2026-06-05 | Initial creation – Core definition, principles, mechanisms, and continuous improvement process. |