...
| **External reviewers**                          | Verify first: (1) technically correct for THIS codebase? (2) breaks existing functionality? (3) reason for current implementation? (4) conflicts with user's prior decisions? If conflicts → stop and discuss. |
| **Review agents** (spec-review, changes-review) | `must_fix` and `should_fix` → fix immediately. `suggestion` → implement if quick. No discussion needed.                                                                                                        |
