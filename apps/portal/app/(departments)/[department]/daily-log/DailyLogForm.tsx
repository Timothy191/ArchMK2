...
export function DailyLogForm({ departmentId, machines }: { departmentId: string; machines: Machine[] }) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      {success && (
        <GlassCard className="border-emerald-500/30 bg-emerald-500/5">
          <p className="text-emerald-400 text-sm font-medium">Daily log submitted successfully</p>
        </GlassCard>
      )}
      ...
      {/* Shift & Date */}
      <GlassCard>
        <h3 className="text-sm font-semibold text-white/70 uppercase tracking-wider mb-4">Shift Details</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm text-white/50 mb-1">Date</label>
            <input
              type="date" name="log_date" defaultValue={today} required
      ...
      {/* Machine Hours */}
      <GlassCard>
        <h3 className="text-sm font-semibold text-white/70 uppercase tracking-wider mb-4">Machine Hours</h3>
        <div className="space-y-3">
          {machines.map((machine, idx) => (
            <div key={machine.id} className="flex items-center gap-4">
              ...
              <input type="hidden" name={`machine_hours[${idx}].machine_id`} value={machine.id} />
              <div className="w-32">
                <input
                  type="number" name={`machine_hours[${idx}].hours_worked`}
                  min="0" max="24" step="0.5" defaultValue="0"
          ...
          {machines.length === 0 && (
            <p className="text-white/30 text-sm">No active machines for this department.</p>
          )}
        </div>
      </GlassCard>
      ...
      {/* Fuel Logs */}
      <GlassCard>
        <h3 className="text-sm font-semibold text-white/70 uppercase tracking-wider mb-4">Diesel Consumption</h3>
        <div className="space-y-3">
          {machines.map((machine, idx) => (
            <div key={`fuel-${machine.id}`} className="flex items-center gap-4">
              ...
              <input type="hidden" name={`fuel_logs[${idx}].machine_id`} value={machine.id} />
              <div className="w-32">
                <input
                  type="number" name={`fuel_logs[${idx}].diesel_litres`}
                  min="0" step="0.1" defaultValue="0"
                  className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-right focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                />
              </div>
      ...
      {/* Production */}
      <GlassCard>
        <h3 className="text-sm font-semibold text-white/70 uppercase tracking-wider mb-4">Production</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm text-white/50 mb-1">Coal Removed (tonnes)</label>
            <input
              type="number" name="coal_tonnes" min="0" step="0.01" defaultValue="0"
              className="w-full px-4 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50"
            />
          </div>
          <div>
            <label className="block text-sm text-white/50 mb-1">Waste Removed (tonnes)</label>
            <input
              type="number" name="waste_tonnes" min="0" step="0.01" defaultValue="0"
              className="w-full px-4 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50"
            />
          </div>
        </div>
      ...
      <div className="flex justify-end gap-3">
        <button
          type="button" onClick={() => router.push(`/${departmentId}`)}
