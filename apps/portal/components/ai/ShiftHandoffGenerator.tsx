...
export function ShiftHandoffGenerator({ shiftData, onGenerate }: ShiftHandoffGeneratorProps) {
...
${shiftData.incidents.join("\n") || "None"}
      ...
      const prompt = AIPrompts.shiftHandoff(data);
      ...
      setError("Failed to generate report. AI service may be temporarily unavailable.");
    } finally {
  ...
  return (
    <GlassCard className="p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-lg font-medium text-[#fafafa]">Shift Handoff Report</h3>
      ...
      {!report && !isGenerating && !error && (
        <div className="text-center py-6 text-[#898989]">
          <p className="text-sm">Generate an AI-powered shift handoff report for the next shift</p>
          <p className="text-xs mt-2">Includes key accomplishments, ongoing issues, and priorities</p>
        </div>
      )}
      ...
      <div className="mt-4 pt-4 border-t border-[#363636]">
        <p className="text-xs text-[#898989]">
          AI analyzes shift activities, delays, and incidents to create comprehensive handoff reports.
        </p>
      </div>
    </GlassCard>
  );
