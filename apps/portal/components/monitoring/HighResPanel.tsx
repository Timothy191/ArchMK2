    ...
    <div className="space-y-4">
      ...
      <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-emerald-400">High-Resolution Commercial Imagery</p>
      ...
      <div className="p-4 rounded-xl bg-[#171717] border border-[#363636]">
        ...
        <p className="text-[10px] text-[#898989] mb-3">
          Conical approximation from DEM-derived base + peak elevation and mapped area.
          For certified survey use LiDAR or photogrammetric point cloud.
        </p>
        <div className="grid grid-cols-2 gap-2 mb-3">
          <div>
            <label className="text-[10px] text-[#898989] block mb-1">Base elevation (m)</label>
            <input
              type="number"
              value={baseElev}
          ...
          <div>
            <label className="text-[10px] text-[#898989] block mb-1">Peak elevation (m)</label>
            <input
              type="number"
              value={peakElev}
          ...
          <div>
            <label className="text-[10px] text-[#898989] block mb-1">Footprint area (ha)</label>
            <input
              type="number"
              value={areaHa}
          ...
          <div>
            <label className="text-[10px] text-[#898989] block mb-1">Material type</label>
            <select
              value={material}
        ...
        {stockpileResult ? (
          ...
          <p className="text-[10px] text-amber-400">Peak elevation must be greater than base elevation.</p>
        )}
      </div>
      ...
      <div className="p-4 rounded-xl bg-[#171717] border border-[#363636]">
        ...
        <p className="text-[10px] text-[#898989] mb-3">
          Select two dates to compare imagery and detect stockpile movement, new infrastructure,
          or tailings pond level changes.
        </p>
        <div className="grid grid-cols-2 gap-2 mb-3">
          <div>
            <label className="text-[10px] text-[#898989] block mb-1">From date</label>
            <input
              type="date"
              value={cdFrom}
          ...
          <div>
            <label className="text-[10px] text-[#898989] block mb-1">To date</label>
            <input
              type="date"
              value={cdTo}
        ...
        {cdFrom && cdTo && cdTo > cdFrom ? (
          <div className="p-2 bg-[#3ecf8e]/10 border border-[#3ecf8e]/20 rounded-lg">
            <p className="text-[11px] text-[#3ecf8e]">
              ✓ Period selected: {Math.round((new Date(cdTo).getTime() - new Date(cdFrom).getTime()) / 86400000)} days
            </p>
          ...
          <p className="text-[10px] text-amber-400">"To" date must be after "From" date.</p>
        ) : null}
      </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-[#242424]">
                <th className="text-left p-2.5 text-[#898989] font-medium">Provider</th>
                <th className="text-left p-2.5 text-[#898989] font-medium">GSD</th>
                <th className="text-left p-2.5 text-[#898989] font-medium">Revisit</th>
                <th className="text-left p-2.5 text-[#898989] font-medium">Cost</th>
              </tr>
            </thead>
            <tbody>
              {PROVIDERS.map((p, i) => (
                ...
                >
                  ...
                  <td className="p-2.5 text-[#b4b4b4] text-[11px]">{p.resolution}</td>
                  <td className="p-2.5 text-[#b4b4b4] text-[11px]">{p.revisit}</td>
