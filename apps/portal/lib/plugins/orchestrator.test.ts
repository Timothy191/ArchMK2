...
describe("PluginOrchestrator.executeEngine", () => {
  it("throws NotFoundError for unknown plugin", async () => {
    ...
    await expect(pluginOrchestrator.executeEngine("nonexistent-plugin")).rejects.toThrow(NotFoundError);
  });
  ...
  it("throws NotFoundError with plugin ID in message", async () => {
    ...
    await expect(pluginOrchestrator.executeEngine("some-missing-id")).rejects.toThrow("some-missing-id");
  });
});
...
describe("PluginOrchestrator.triggerHook", () => {
  it("resolves without error when no plugins are loaded", async () => {
    ...
    await expect(pluginOrchestrator.triggerHook("onLogCreated", { shift: "day" })).resolves.toBeUndefined();
  });
});
...
describe("PluginOrchestrator.executeEngine – success", () => {
  it("executes plugin engine and returns result", async () => {
    ...
    const mockPlugin = {
      metadata: { id: "mock-plugin", name: "Mock", version: "1.0.0", enabled: true },
      engine: mockEngine,
    };
    (pluginOrchestrator as any).activePlugins.set("mock-plugin", mockPlugin);

    const result = await pluginOrchestrator.executeEngine("mock-plugin", { rpm: 1000 });
  ...
  it("wraps engine runtime error in APIError", async () => {
    ...
    const mockEngine = { execute: jest.fn().mockRejectedValue(new Error("computation failed")) };
    const mockPlugin = {
      metadata: { id: "crash-plugin", name: "Crash", version: "1.0.0", enabled: true },
      engine: mockEngine,
    };
    (pluginOrchestrator as any).activePlugins.set("crash-plugin", mockPlugin);
...
describe("PluginOrchestrator.triggerHook – with hooks", () => {
  it("calls hooks on active plugins", async () => {
    ...
    const mockPlugin = {
      metadata: { id: "hook-plugin", name: "Hook", version: "1.0.0", enabled: true },
      hooks: { onLogCreated: hookFn },
    };
    (pluginOrchestrator as any).activePlugins.set("hook-plugin", mockPlugin);
  ...
  it("isolates hook crashes without throwing", async () => {
    ...
    const mockPlugin = {
      metadata: { id: "bad-hook-plugin", name: "BadHook", version: "1.0.0", enabled: true },
      hooks: { onLogCreated: hookFn },
    };
    (pluginOrchestrator as any).activePlugins.set("bad-hook-plugin", mockPlugin);
...
describe("PluginOrchestrator.getActiveWidgets – with widgets", () => {
  it("returns widgets from active plugins", async () => {
    ...
    const mockWidget = { id: "w1", name: "Dashboard", component: "DashboardWidget" };
    const mockPlugin = {
      metadata: { id: "widget-plugin", name: "Widget", version: "1.0.0", enabled: true },
      widgets: [mockWidget],
    };
    (pluginOrchestrator as any).activePlugins.set("widget-plugin", mockPlugin);
