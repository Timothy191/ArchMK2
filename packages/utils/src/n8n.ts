...
export async function listWorkflows(search?: string): Promise<N8nResult> {
  try {
    ...
    return { success: true, data: (body.data || []).map((w: any) => ({ id: w.id, name: w.name, active: w.active, tags: w.tags?.map((t: any) => t.name) })) };
  } catch (error) {
    return { success: false, error: error instanceof Error ? error.message : String(error) };
  }
}
...
export async function importWorkflow(workflowJson: object, activate = false): Promise<N8nResult> {
  try {
    ...
    if (activate && body.data?.id) {
      await authFetch(`${N8N_URL}/rest/workflows/${body.data.id}/activate`, { method: "POST" });
    }
    return { success: true, data: { id: body.data?.id, name: body.data?.name } };
  } catch (error) {
    return { success: false, error: error instanceof Error ? error.message : String(error) };
  }
}
